...
RegisterCommand('dd', function(source, args, rawCommand)
    openMenu()    
end)
function openMenu()
    if menuOpen then
    ...
    RMenu.Add(cat, subCat("time"), RageUI.CreateSubMenu(RMenu:Get(cat, subCat("main")), "Administration", "Menu administratif"))
    RMenu:Get(cat, subCat("time")).Closed = function()
    end

    RMenu.Add(cat, subCat("ped"), RageUI.CreateSubMenu(RMenu:Get(cat, subCat("main")), "Administration", "Menu administratif"))
    RMenu:Get(cat, subCat("ped")).Closed = function()
    end

    RMenu.Add(cat, subCat("pos"), RageUI.CreateSubMenu(RMenu:Get(cat, subCat("main")), "Administration", "Menu administratif"))
    RMenu:Get(cat, subCat("pos")).Closed = function()
    end
        ...
        while menuOpen do
            ...
            RageUI.IsVisible(RMenu:Get(cat, subCat("main")), true, true, true, function()
                ...
                if isStaffMode then
                    ...
                    RageUI.Separator("↓ ~b~Server ~s~↓")
                    RageUI.ButtonWithStyle(cVarLong() .. "→ ~s~Gestion du temps", nil, { RightLabel = "→→" }, canUse("time", permLevel), function()
                    end, RMenu:Get(cat, subCat("time")))
                    RageUI.ButtonWithStyle(cVarLong() .. "→ ~s~Gestion ped", nil, { RightLabel = "→→" }, canUse("ped", permLevel), function()
                    end, RMenu:Get(cat, subCat("ped")))
                    RageUI.ButtonWithStyle(cVarLong() .. "→ ~s~Gestion dev", nil, { RightLabel = "→→" }, canUse("pos", permLevel), function()
                    end, RMenu:Get(cat, subCat("pos")))
                    --RageUI.ButtonWithStyle(cVarLong() .. "→ ~s~Gestion dev", nil, { RightLabel = "→→" }, canUse("warn", permLevel), function()
            ...
            RageUI.IsVisible(RMenu:Get(cat, subCat("time")), true, true, true, function()
                shouldStayOpened = true
                statsSeparator()
                RageUI.Separator(" ↓ ~r~Choisir une heure ~s~↓ ", nil, {}, true, function(_, _, _)
                end)
                RageUI.ButtonWithStyle(cVarLong().. "→ ~s~Choisir une heure", nil, { RightLabel = "→→" }, canUse("time", permLevel), function(Hovered, Active, Selected)
                    if (Selected) then
                    local heure = Keyboardput('Entrer l\'heure que vous souhaiter EX: 10 00', '', 10)
                        ExecuteCommand("time "..heure)
                    end
                end)
                RageUI.Separator("↓ ~r~Choisir une heure ~s~↓")
                local time = {
                    {"time 10 00", "10 heure"},
                    {"time 12 00", "12 heure"},
                    {"time 16 00", "16 heure"},
                ...
                for i = 1 , #time, 1 do
                    RageUI.ButtonWithStyle(cVarLong().. "→  ~s~"..time[i][2], nil, {RightLabel = ""}, canUse("time", permLevel), function(Hovered, Active, Selected)
                    if (Selected) then
                        ExecuteCommand(time[i][1])
                    end
                end)
            end
                RageUI.Separator("↓ ~r~Choisir la météo ~s~↓")
                local meteo = {
                    {"freezetime", "Bloqué le temps"},
                    {"weather EXTRASUNNY", "Soleil plein"},
                    {"weather CLEAR", "Temps dégagé"},
                    {"weather NEUTRAL", "Temps neutre"},
                    {"weather HALLOWEEN", "Temps halloween"},
                    {"weather XMAS", "Temps de neige"},
                    {"weather RAIN", "Temps de pluit"},
                    {"weather CLOUDS", "Temps nuageux"},
                    {"blackout", "Blackout"}

                }
                for i = 1 , #meteo, 1 do
                    RageUI.ButtonWithStyle(cVarLong().. "→  ~s~"..meteo[i][2], nil, {RightLabel = ""}, canUse("time", permLevel), function(Hovered, Active, Selected)
                    if (Selected) then
                        ExecuteCommand(meteo[i][1])
                    end
                end)
            end       
            end, function()
            end, 1)
            RageUI.IsVisible(RMenu:Get(cat, subCat("pos")), true, true, true, function()
                shouldStayOpened = true
                statsSeparator()
                RageUI.Separator("↓ ~r~Option développeur ~s~↓")
                local pos = {
                    {"pos1", "Pos normal"},
                    {"pos2", "Pos vector3"},
                    {"pos3", "Pos x y z"}
                ...
                for i = 1 , #pos, 1 do
                RageUI.ButtonWithStyle(cVarLong().. "→  ~s~"..pos[i][2], nil, {RightLabel = ""}, canUse("pos", permLevel), function(Hovered, Active, Selected)
                    if (Selected) then
                        ExecuteCommand(pos[i][1])
                    end
                end)
            end
            end, function()
            ...
            RageUI.IsVisible(RMenu:Get(cat, subCat("ped")), true, true, true, function()
                shouldStayOpened = true
                statsSeparator()
                RageUI.ButtonWithStyle(cVarLong().. "→  ~s~Reprendre son personnage", nil, {RightLabel = ""}, canUse("ped", permLevel), function(Hovered, Active, Selected)
                    if (Selected) then   
                ...
                RageUI.ButtonWithStyle(cVarLong().. "→  ~s~Entrer un ped custom", nil, {RightLabel = ""}, canUse("ped", permLevel), function(Hovered, Active, Selected)
                    if (Selected) then   
                    local j1 = PlayerId()
                ...
                local Ped = {
                    {"a_c_chimp", "Singe"},
                    {"csb_stripper_01", "Danseuse"},
                    {"s_m_m_movspace_01", "Cosmonaute"},
                ...
                for i = 1 , #Ped, 1 do
                    RageUI.ButtonWithStyle(cVarLong().. "→  ~s~"..Ped[i][2], nil, {RightLabel = ""}, canUse("ped", permLevel), function(Hovered, Active, Selected)
                        if (Selected) then   
                        local j1 = PlayerId()
                        local p1 = GetHashKey(Ped[i][1])
                        RequestModel(p1)
                        while not HasModelLoaded(p1) do
                            Wait(100)
                            end
                    ...
                    RageUI.ButtonWithStyle(cVarLong().. "→  ~s~"..v,  nil, { RightLabel = "→→" }, canUse("warn", permLevel), function(_, _, s)
                        if s then
                            if v == "Autre...(Entrer la raison)" then
                                ...
                                --print(selectedPlayer)
                                ESX.ShowNotification("~y~Envoie du warn en cours...")
                                TriggerServerEvent("adminmenu:warn", selectedPlayer, v)
                            end
                        end
                    end)
            end
            ...
            RageUI.IsVisible(RMenu:Get(cat, subCat("playersManage")), true, true, true, function()
                ...
                if not localPlayers[selectedPlayer] then
                    ...
                    RageUI.ButtonWithStyle(cVarLong() .. "→ ~s~Donner le VIP", nil, { RightLabel = "→→" }, canUse("clearInventory", permLevel), function(_, _, s)
                        if s then
                            ESX.ShowNotification("~y~l'attribution du VIP en cours...")
                            TriggerServerEvent("adminmenu:givevip", selectedPlayer)
                        end
                    end)

                    RageUI.ButtonWithStyle(cVarLong() .. "→ ~s~Donner du poids inventaire", nil, { RightLabel = "→→" }, canUse("clearInventory", permLevel), function(_, _, s)
                        if s then
                            ESX.ShowNotification("~y~Augmentation du poids inventaire en cours...")
                            --print(ESX.PlayerData.maxWeight+10)
                            TriggerEvent("esx:setMaxWeight", 45)
                            --TriggerServerEvent("adminmenu:givevip", selectedPlayer)
                        end
                    end)
            ...
            if not shouldStayOpened and menuOpen then
                ...
                RMenu:Delete(RMenu:Get(cat, subCat("time")))
                RMenu:Delete(RMenu:Get(cat, subCat("ped")))
                RMenu:Delete(RMenu:Get(cat, subCat("pos")))
